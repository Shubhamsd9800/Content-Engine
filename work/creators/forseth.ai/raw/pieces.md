# pieces — forseth.ai
Run: 2026-08-30   Posts in CSV: 10   Pinned: 0   Scored: 10
Median engagement: 566.0   Date span: 49 days
GAP: 34.68
Handed to creator-analyst: 10 pieces — 3 WINNER, 7 NORMAL

## WINNERS — top 3 by engagement   ← these get transcribed

### #6  ·  7/24/2026  ·  19627  ·  34.68x
likes 6334  comments 13293  engagement 19627
caption (truncated at 100 chars by OpenCLI — for FINDING the post, never for analysis):
          comment "Kallaway" and i'll send the breakdown over 👇  Day 1 of recreating editing styles with Clau
URL:      https://www.instagram.com/reel/DbJ0mp1sbGL/
FULL CAPTION:
          comment "Kallaway" and i’ll send the breakdown over 👇
          
          Day 1 of recreating editing styles with Claude Code to show how easy it is to edit HIGH QUALITY edits with my claude code workflow

### #7  ·  7/13/2026  ·  7839  ·  13.85x
likes 2521  comments 5318  engagement 7839
caption (truncated at 100 chars by OpenCLI — for FINDING the post, never for analysis):
          comment "edit" and i'll send the breakdown over👇  Day 4 of letting claude code edit every video I p
URL:      https://www.instagram.com/reel/Datg7C0M9Gp/
FULL CAPTION:
          comment "edit" and i’ll send the breakdown over👇
          
          Day 4 of letting claude code edit every video I post

### #5  ·  7/28/2026  ·  4292  ·  7.58x
likes 1755  comments 2537  engagement 4292
caption (truncated at 100 chars by OpenCLI — for FINDING the post, never for analysis):
          comment "DAY 5" and i'll send the breakdown on how this works.  this entire video was edited by clau
URL:      https://www.instagram.com/reel/DbT4_N_u-zG/
FULL CAPTION:
          comment "DAY 5" and i’ll send the breakdown on how this works.
          
          this entire video was edited by claude code. i didnt even touch it ONCE. And it is getting better every signøe time I post

## NORMAL — the control set   ← captions and numbers only, no transcript

| # | date | ratio | likes | comments | caption |
|---|---|---|---|---|---|
| 1 | 8/7/2026 | 3.11x | 550 | 1208 | comment «SD» for the claude editing pipeline breakdown!👇  THIS ONE is insane, breaking it all down |
| 2 | 8/5/2026 | 0.90x | 163 | 349 | comment "DAN" and i'll send you the breakdown👇 |
| 3 | 8/3/2026 | 0.18x | 50 | 50 | comment "BREAKDOWN" to see how the workflow👇 |
| 4 | 8/3/2026 | 0.30x | 83 | 84 | Comment "breakdown" to edit with this pipeline  You should focus on recording ans doing what you are |
| 8 | 7/7/2026 | 0.98x | 160 | 397 | comment "pipeline" and i'll send you the full breakdown.  day one of letting claude code edit everyt |
| 9 | 6/26/2026 | 0.58x | 128 | 202 | comment "edit" and i'll send you the full breakdown i did!  this entire video cost me $600 to make. |
| 10 | 6/19/2026 | 1.02x | 189 | 386 | Comment "Carl" and I'll send you the full breakdown.  I don't edit my videos anymore. Claude Code do |

## 📌 PINNED — the creator's own picks, excluded from median and top 3
| # | date | age | engagement | caption |
|---|---|---|---|---|
| — | — | — | — | none — dates decrease (non-strictly) index 1→10 |

> Not scored. Pinned posts are pre-selected by the creator and usually predate the account's
> current size. Read them in the teardown for what he chose to pin; never as a discovery.

## WATCH — ranks 4 and 5, next in line

| # | date | engagement | ratio | caption |
|---|---|---|---|---|
| 1 | 8/7/2026 | 1758 | 3.11x | comment «SD» for the claude editing pipeline breakdown!👇  THIS ONE is insane, breaking it all down |
| 10 | 6/19/2026 | 575 | 1.02x | Comment "Carl" and I'll send you the full breakdown.  I don't edit my videos anymore. Claude Code do |

## RUN NOTES
Fresh write. Only 10 posts returned by OpenCLI, not the usual 12 — the profile grid loaded
fewer. Sample floor (8 unpinned) cleared. No pinned posts — dates only decrease or tie (index
3/4 both 8/3) across the whole grid. No post is under 7 days old, so no 🌱 flags this run — the
whole set is 3+ weeks old. GAP=34.68 is unusually high (recorded, not acted on) — #6 (7/24)
carries roughly 8x the engagement of anything else in the set.
